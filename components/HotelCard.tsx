...
import { Card, CardContent, Typography, Box, Button, Rating, Chip, Stack } from '@mui/material'
import { LocationOn, WhatsApp, Phone } from '@mui/icons-material'
  ...
  return (
    ...
    >
      <Box sx={{ position: 'relative', height: 220 }}>
        ...
        >
          <Rating value={Number(hotel.rating)} precision={0.1} readOnly size="small" />
      ...
      <CardContent sx={{ flexGrow: 1, p: 3 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 1 }}>
          <Typography variant="h6" component="h2" fontWeight={800} sx={{ lineHeight: 1.2 }}>
            {hotel.name}
          </Typography>
        </Stack>
        ...
        <Stack direction="row" spacing={1} sx={{ mt: 2 }}>
          ...
          <Button
            ...
            variant="outlined"
            startIcon={<Phone />}
            onClick={(e) => { e.stopPropagation(); window.location.href = `tel:${callNumber}`; }}
            sx={{ 
                borderRadius: '12px', 
                textTransform: 'none', 
                fontWeight: 600

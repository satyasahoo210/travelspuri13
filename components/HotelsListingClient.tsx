...
import { Close, FilterList, Search, Tune } from '@mui/icons-material'
import {
  Box,
  Button,
  Chip,
...
export default function HotelsListingClient({
  initialHotels,
}: HotelsListingClientProps) {
  ...
  const filteredHotels = useMemo(() => {
    ...
    let result = data.filter((hotel: Hotel) => {
      ...
      const matchesPrice =
        hotel.starting_price >= filters.priceRange[0] &&
        hotel.starting_price <= filters.priceRange[1]
      const matchesRating = Number(hotel.rating) >= filters.rating
      const matchesAmenities =
        filters.amenities.length === 0 ||
        filters.amenities.every((a) =>
          hotel.amenities_search?.toLowerCase().includes(a.toLowerCase()),
        )
